      ...
      <template v-for="(item , index) in items">
        <tr style="text-align: center;vertical-align: middle;cursor: move;height: 50px;line-height: 50px"
          ...
          <td :style="{width: '30%',verticalAlign: 'middle',paddingLeft:level*20+'px'}">
            ...
            <el-col class="col" :span="22">
              ...
              <el-input style="width: 90%;" placeholder="请填写名称" v-model.trim="item.name"
                        v-if="item.name!=null && (level!=0 || type!=1)" :disabled="false">
              ...
              <el-input style="width: 90%;" placeholder="该字段没有名称" v-else>
                <el-popover slot="append" placement="bottom" width="100" trigger="hover"
                            v-if="(item.type==0 || item.type==1) && statusExist">
                  <el-row class="row" style="font-size: 15px;color: #20A0FF;text-align: center">
                    {{statusValid(item)}}
          ...
          <td style="width: 18%">
            ...
            <!--placeholder="请填写备注;" v-model="item.remark" ></el-input>-->
          </td>
          <td style="width: 30%">
            <!--<el-input type="textarea" style="width: 90%;height: 46px;line-height: 46px" resize="none" :rows="0"-->
            <!--placeholder="请填写Mock数据;" v-model="item.mock" v-if="item.type!=4" ></el-input>-->
          </td>
        </tr>
        <tr v-if="(item.type==4 || item.type==3) && (item.data && item.data.length>0) && item.show">
          <td colspan="7" style="width: 100%;margin:0;padding: 0 ">
            <json-edior :source="item.data" :le="level+1" :parent="item"></json-edior>
          </td>
        </tr>
      </template>
    </table>
  ...
  export default {
    ...
    data: function () {
      return {
        ...
        itemType: 0,
        testData: {name: 'title', num: 3, arr: [1, 2, 34]},
        items: [],
      }
    },
    computed: {
    ...
    methods: {
      parsedItems() {
        ...
        for (let key in this.source) {
          let val = this.source[key]
          let tp = typeof(val)
          console.log(key, '=', val)
          let item = {name: key, data: val}
          ...
          switch (tp) {
            ...
            default:
              ...
              for (let ikey in val) {
                if (typeof(val[ikey]) === 'number') {
                  isNumber = true
                }
                if (typeof(val[ikey]) === 'string') {
                  isString = true
                }
              }
