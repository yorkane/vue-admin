<template>
  <el-table :data="vModelData" border v-loading="loading" stripe
    ...
    <el-table-column label="操作" width="240" fixed="right"><!--menu opts here-->
      ...
      <template v-if="getColWidth(item) > 0">
        <el-table-column :label="item.Comment" min-width="120px"
          ...
          <template slot-scope="scope">
            ...
            <i style="font-size: 24px;color: #1f2d3d" v-else-if="item.Field=='icon'"
  ...
  export default {
    ...
    methods: {
      ...
      getDragDropId: function (index, rowData) {
        return 'dragdrop_' + rowData[this.dataStruct._PK] + '_' + index;
      },
      copyData: function (obj) {
        var ori = {};
        for (var key in obj) {
          var val = obj[key];
          var fi = this.dataStruct._FIELD_DIC[key];
          if (fi || key == '__children') {
            ori[key] = val;
          }
        }
        return ori;
...
//            if(toIndex === this.nodeList.length)
        ...
        if ((fromIndex - toIndex) == 1 && fromObj.order != 0) { // if too nodes side by side
//					toIndex--;
        }
      ...
      switchValue: function (key, row) {
        let obj = {};
        let pk = this.dataStruct._PK;
        obj[pk] = row[pk];
        obj[key] = row[key];
        this.$emit('switch-value', obj)
      },
      ...klib
    }
  }
