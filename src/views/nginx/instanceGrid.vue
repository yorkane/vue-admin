          ...
          <template slot-scope="scope">
            <template v-if="item.isPK && hasDragOrder()">
              <div draggable :id="getDragDropId(scope.$index, scope.row)" :orderindex="scope.$index"
                ...
                <a class="menuHref">{{scope.row[dataStruct._PK]}}</a>
              </div>
            </template>
            <el-tag v-else-if="item.isStatus" :type="getTagType(scope.row[item.Field])" size="small" hit>
              {{getStatus(item.Field,
              scope.row[item.Field], true)}}
            </el-tag>
            <el-tag v-else-if="item.isOption||item.isTextOption" :type="getTagType(scope.row[item.Field])" size="small" hit>
...
<script>
  ...
  export default {
    ...
    data: function () {
      return {
        columnWidth: {},
        tags: {0: 'info', 1: 'success', 2: 'danger', 3: 'warning', 4: 'primary', 5: 'info', 6: 'info', 7: ''}
      }
    },
    filters: {},
    watch: {},
