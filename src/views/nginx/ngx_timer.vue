  ...
  <div class="mainPanel">
    ...
    <div style="overflow: auto;">
      <k-condition ref="condition" size="mini" :dataStruct="m_dataStruct" @btnEvt_insert="handleEvent"
                   @btnEvt_refresh="getData()" @query="getData">
      </k-condition>
...
<script>
  import timerAPI from '../../api/nginx_timer'
  ...
  export default {
    ...
    methods: {
      refresh() {
        ...
        let that = this
        setTimeout(() => {
          //console.log('refreshing getData method...')
          that._api.getData({}, true).then(data => {
            that.dataList = data
            that.gridData = data.list
            that.totalCount = data.count
            that.refresh_interval = data.count > 0 ? 10000 : 30000
            that.refresh()
          })
        }, this.refresh_interval)
        // console.debug('setTimeout created ', this.refresh_interval)
      },
      ...
      handleEvent(data, index, eventName) {
        ...
        switch (eventName) {
          ...
          case 'btnEvt_delete':
            ...
            this.api.deleteById(data[this.m_dataStruct._PK]).then(resp => {
